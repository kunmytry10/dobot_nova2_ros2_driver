...
DATA_ROOT ?= $(WS)/data
HANDEYE_DATASET_ROOT ?= $(DATA_ROOT)/handeye/datasets
HANDEYE_DATASET_NAME ?=
HANDEYE_SAMPLES_DIR ?= $(DATA_ROOT)/handeye/samples
HANDEYE_RESULT_FILE ?=
HANDEYE_DIAGNOSE_FILE ?=
HANDEYE_STATIC_TF_FILE ?= $(DATA_ROOT)/handeye/handeye_result.yaml
...
JOY_DATASET_ROOT ?= $(DATA_ROOT)/collections/move_jog
...
COLLECTION_REQUIRE_START_POSE ?= false
COLLECTION_START_POSE_FILE ?= $(JOY_DATASET_ROOT)/servo_p_start_pose.json
COLLECTION_START_JOINT_TOLERANCE_DEG ?= 1.0
COLLECTION_START_GRIPPER_TOLERANCE_MM ?= 3.0
COLLECTION_PREPARE_SPEED ?= 10
COLLECTION_PREPARE_ACCELERATION ?= 10
COLLECTION_PREPARE_TIMEOUT_SEC ?= 30.0
SERVO_DATASET_ROOT ?= $(DATA_ROOT)/collections/servo_p_v2
SERVO_LEROBOT_DATASET_ROOT ?= $(SERVO_DATASET_ROOT)/lerobot_pi05_servo_p_v2
SERVO_LEROBOT_REPO_ID ?= local/dobot_nova2_tape_pi05_servo_p_v2
SERVO_START_POSE_FILE ?= $(SERVO_DATASET_ROOT)/servo_p_start_pose.json
...
SYSTEM_LOCK_FILE ?= /tmp/dobot_nova2_system_$(shell id -u).lock
...
build:
	$(ROS_SETUP) && $(ORBBEC_ENV) && cd $(WS) && colcon build --symlink-install --packages-up-to dobot_camera dobot_handeye dobot_keyboard dobot_joy dobot_ros2 dobot_policy
...
data-set-start:
	$(ROS_ENV) && ros2 service call /data_collection/set_start_pose std_srvs/srv/Trigger "{}"

data-prepare:
	$(ROS_ENV) && ros2 service call /data_collection/prepare std_srvs/srv/Trigger "{}"

data-clear-start:
	$(ROS_ENV) && ros2 service call /data_collection/clear_start_pose std_srvs/srv/Trigger "{}"

data-start-pose-status:
...
policy-motion-test:
	POLICY_ARMED=true POLICY_MOTION_ONLY=true POLICY_RUN_TIMEOUT_SEC=40 $(WS)/scripts/run_pi05_grasp.sh

policy-motion-full:
	POLICY_ARMED=true POLICY_MOTION_ONLY=true POLICY_MOTION_TEST_DURATION_SEC=45 POLICY_MAX_EPISODE_SEC=75 POLICY_RUN_TIMEOUT_SEC=100 $(WS)/scripts/run_pi05_grasp.sh

# Repeatable motion-only demonstration from the recorded ServoP collection pose.
policy-motion-demo:
	POLICY_ARMED=true POLICY_MOTION_ONLY=true POLICY_START_POSE_FILE=$(SERVO_START_POSE_FILE) POLICY_MOTION_TEST_DURATION_SEC=45 POLICY_MAX_EPISODE_SEC=75 POLICY_RUN_TIMEOUT_SEC=100 POLICY_COUNTDOWN_SEC=3 POLICY_SKIP_BUILD=true POLICY_REUSE_SERVER=true POLICY_KEEP_SERVER=true $(WS)/scripts/run_pi05_grasp.sh

policy-demo-stop:
	@POLICY_STOP_SERVER_ONLY=true $(WS)/scripts/run_pi05_grasp.sh

policy-status:
	$(ROS_ENV) && ros2 service call /dobot_policy/status std_srvs/srv/Trigger "{}"

policy-stop:
	$(ROS_ENV) && ros2 service call /dobot_policy/stop std_srvs/srv/Trigger "{}"
